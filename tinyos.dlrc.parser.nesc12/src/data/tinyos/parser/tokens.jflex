...
import tinyos.dlrc.nesc12.collector.sym;
#else
package tinyos.dlrc.nesc12.lexer;

import tinyos.dlrc.nesc12.parser.sym;
#endif
...
%state INCLUDE_INTERFACE
%state INCLUDE_COMPONENT
...
// NesC
	...
	"module"		{ yybegin( COMPONENT ); }
	"component"		{ yybegin( COMPONENT ); }
	"configuration"	{ yybegin( COMPONENT ); }
	"implementation"	{ yybegin( IMPLEMENTATION ); }
	"generic"		{ /* ignore */ }
	"includes"		{ yybegin( INCLUDES ); }
#else
...
<COMPONENT>{
	{string_literal}	{ /* ignore */ }
	"uses"				{ yybegin( INCLUDE_INTERFACE ); afterReadInclusion = COMPONENT; }
	"provides"			{ yybegin( INCLUDE_INTERFACE ); afterReadInclusion = COMPONENT; }
...
<INCLUDE_INTERFACE>{
	{string_literal}	{ /* ignore */ }
	"interface"			{ yybegin( READ_IDENTIFIER ); afterReadIdentifier = INCLUDE_INTERFACE; }
	"{"					{ nestedIntern++; }
	"}"					{ 	nestedIntern--;
							if( nestedIntern <= 0 ){
								nestedIntern = 0;
...
<IMPLEMENTATION>{
	{string_literal}	{ /* ignore */ }
	"components"		{ yybegin( INCLUDE_COMPONENT ); afterReadInclusion = IMPLEMENTATION; }
...
<INCLUDES>{
	";"					{ yybegin( YYINITIAL ); }
	{identifier}		{ return token( sym.INCLUDE_FILE ); }
	{newline}			{ /* ignore */ }
	{whitespace}		{ /* ignore */ }
	.					{ /* ignore */ }
}	
...
<READ_IDENTIFIER>{
	{identifier}		{ yybegin( afterReadIdentifier ); return token( sym.INCLUDE_INTERFACE ); }
	{newline}			{ /* ignore */ }
	{whitespace}		{ /* ignore */ }
	.					{ /* ignore */ }
}
