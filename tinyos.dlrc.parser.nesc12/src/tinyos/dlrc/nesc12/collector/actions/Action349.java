...
public final class Action349 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION error 
            {
		...
		Token rr = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		 RESULT = new Configuration(true,parser.errorNode( "configuration", "identifier" ),null,null,parser.missing( "configuration", "provides/uses block", rr.getRight()),parser.missing( "configuration", "implementation", rr.getRight())); 			RESULT.setLeft( rl.getLeft() ); 
