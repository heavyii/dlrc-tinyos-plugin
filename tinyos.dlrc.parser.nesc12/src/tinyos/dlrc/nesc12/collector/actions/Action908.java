...
public final class Action908 implements ParserAction{
 ...
 // c_direct_declarator_no_hidden_typedef_name ::= c_identifier 
            {
              Declarator RESULT =null;
		Identifier i = (Identifier)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 				  			if( parser.scopes().isTypedef() ){ 				parser.scopes().addTypedef( i.getToken().getText() ); 			} 			else{ 				parser.scopes().addField( i.getToken().getText() ); 			} 			RESULT = new DeclaratorName( i ); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("c_direct_declarator_no_hidden_typedef_name",103, ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
