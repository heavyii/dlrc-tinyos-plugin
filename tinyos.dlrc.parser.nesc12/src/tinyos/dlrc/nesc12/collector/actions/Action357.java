...
public final class Action357 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION error n_attributes_no_init 
            {
		...
		AttributeList a = (AttributeList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(true,parser.errorNode( "configuration", "identifier" ),null,a,parser.missing( "configuration", "provides/uses block", a.getRange().getRight()),parser.missing( "configuration", "implementation", a.getRange().getRight())); 			RESULT.setLeft( rl.getLeft() ); 
