...
public final class Action392 implements ParserAction{
 ...
 // n_configuration ::= NK_CONFIGURATION error n_component_parameters n_configuration_implementation 
            {
		...
		ConfigurationDeclarationList c = (ConfigurationDeclarationList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(false,parser.errorNode( "configuration", "identifier" ),p,null,parser.missing( "configuration", "provides/uses block", p.getRange().getRight()),c); 			RESULT.setLeft( k.getLeft() ); 
