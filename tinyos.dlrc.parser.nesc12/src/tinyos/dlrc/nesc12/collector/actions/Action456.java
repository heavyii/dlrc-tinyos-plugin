...
public final class Action456 implements ParserAction{
 ...
 // n_binary_component ::= NK_COMPONENT c_curly_open error c_curly_close 
            {
		...
		Token rr = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new BinaryComponent( parser.missing( "binary component", "identifier", rl.getRight() ), null, parser.errorNode( "binary component", "provides/uses block", op.getRight(), rr.getLeft() ) ); RESULT.setLeft( rl.getLeft() ); RESULT.setRight( rr.getRight() ); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("n_binary_component",194, ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-3)), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
