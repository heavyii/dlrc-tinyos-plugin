...
public final class Action356 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION error n_attributes_no_init n_configuration_implementation 
            {
		...
		ConfigurationDeclarationList c = (ConfigurationDeclarationList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(true,parser.errorNode( "configuration", "identifier" ),null,a,parser.missing( "configuration", "provides/uses block", a.getRange().getRight()),c); 			RESULT.setLeft( rl.getLeft() ); 
