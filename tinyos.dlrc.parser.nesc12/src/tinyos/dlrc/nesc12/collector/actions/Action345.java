...
public final class Action345 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION c_identifier n_component_parameters 
            {
		...
		TemplateParameterList p = (TemplateParameterList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(true,i,p,null,parser.missing( "configuration", "provides/uses block", p.getRange().getRight()),parser.missing( "configuration", "implementation", p.getRange().getRight())); 			RESULT.setLeft( rl.getLeft() ); 
