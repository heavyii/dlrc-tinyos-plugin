...
public final class Action126 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION error n_component_parameters 
		...
		TemplateParameterList p = (TemplateParameterList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(true,parser.errorNode( "configuration", "identifier" ),p,null,parser.missing( "configuration", "provides/uses block", p.getRange().getRight()),parser.missing( "configuration", "implementation", p.getRange().getRight())); 			RESULT.setLeft( rl.getLeft() ); 
