...
public final class Action125 implements ParserAction{
 ...
 // n_configuration ::= NK_GENERIC NK_CONFIGURATION error n_component_parameters n_configuration_implementation 
            {
		...
		ConfigurationDeclarationList c = (ConfigurationDeclarationList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(true,parser.errorNode( "configuration", "identifier" ),p,null,parser.missing( "configuration", "provides/uses block", p.getRange().getRight()),c); 			RESULT.setLeft( rl.getLeft() ); 
