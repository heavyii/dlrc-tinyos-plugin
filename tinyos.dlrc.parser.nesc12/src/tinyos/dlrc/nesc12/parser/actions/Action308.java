...
public final class Action308 implements ParserAction{
 ...
 // c_primary_expression ::= c_round_open c_round_close 
            {
		...
		Token rr = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 ParenthesizedExpression result = new ParenthesizedExpression( parser.missing( null, "expression", rl.getLeft(), rr.getRight() ) );
			...
			result.setRanges( rl, rr ); 
              CUP$parser$result = parser.getSymbolFactory().newSymbol("c_primary_expression",31, ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)), ((java_cup.runtime.Symbol)CUP$parser$stack.peek()), RESULT);
            }
