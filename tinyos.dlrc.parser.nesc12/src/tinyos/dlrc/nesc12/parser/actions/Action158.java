...
public final class Action158 implements ParserAction{
 ...
 // n_configuration ::= NK_CONFIGURATION error n_component_parameters 
            {
		...
		TemplateParameterList p = (TemplateParameterList)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
		 RESULT = new Configuration(false,parser.errorNode( "configuration", "identifier" ),p,null,parser.missing( "configuration", "provides/uses block", p.getRange().getRight()),parser.missing( "configuration", "implementation", p.getRange().getRight())); 			RESULT.setLeft( k.getLeft() ); 
