...
public class PointerToPointer implements Conversion{
    public boolean responsible( Type source, Type destination ){
        return source.asPointerType() != null && destination.asPointerType() != null;
    }
    
    public void check( Type source, Type destination, ConversionTable table, ConversionMap map ){
        map.pushPointer();
        table.check( source.asPointerType().getRawType(), destination.asPointerType().getRawType(), map );
        map.popPointer();
    }
}
