...
public class VoidToAny implements Conversion{
    public boolean responsible( Type source, Type destination ){
        return source.asBase() == BaseType.VOID;
    }
    
    public void check( Type source, Type destination, ConversionTable table, ConversionMap map ){
        if( !map.isPointer() ){
            if( destination.asBase() != BaseType.VOID ){
                map.reportError( "void value should be ignored", null, destination );
            }
        }
    }
}
