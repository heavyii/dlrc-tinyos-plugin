...
public class Generic implements Conversion{
    public boolean responsible( Type source, Type destination ){
        return source.asGenericType() != null || destination.asGenericType() != null;
    }
    
    public void check( Type source, Type destination, ConversionTable table, ConversionMap map ){
        if( !table.equals( source, destination )){
            map.reportWarning( "can't check conversions involving generic types", source, destination );
        }
    }
}
