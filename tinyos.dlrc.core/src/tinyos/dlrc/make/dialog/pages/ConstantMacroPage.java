...
public class ConstantMacroPage extends KeyValuePage<MakeMacro>{
    ...
    protected void createTableColumns( Table table ){
        ...
        TableColumn dlrc = new TableColumn( table, SWT.LEFT );
        dlrc.setText( "Eclipse" );
        dlrc.setResizable( true );
        dlrc.setMoveable( false );
        dlrc.setWidth( 75 );
	...
	protected MakeMacro create( String key, String value, TableItem row ){
		...
		boolean dlrc = TRUE.equals( row.getText( 2 ) );
		boolean ncc = TRUE.equals( row.getText( 3 ) );
		return new MakeMacro( macro, dlrc, ncc );
	}

	@Override
	protected void show( MakeMacro entry, TableItem item ){
		item.setText( new String[]{ getKey( entry ), getValue( entry ), getBoolean( entry.isIncludeDlrc() ), getBoolean( entry.isIncludeNcc() ) } );
	}
