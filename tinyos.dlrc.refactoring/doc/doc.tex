...
\hypersetup{colorlinks=false,
            pdftitle={Refactoring extension for the Dlrc NesC Eclipse Plug-in},
            pdfauthor={Noah Heusser, Max Urech},
            pdfsubject={Documentation of the refactoring extension for the Dlrc NesC Eclipse Plug-in},
            pdfkeywords={Refactoring, NesC, Dlrc, TinyOS, Eclipse, LTK, Language Toolkit, ETH},
            pdfborder={0 0 0}
} 
...
% Title Page
%\title{Refactoring extenstion for the Dlrc NesC Eclipse plug-in}
...
The solution is the so called ``refactoring`` plug-in for Eclipse. The plug-in extends the existing Dlrc plug-in for Eclipse.
...
\nomenclature{AST}{AST stands for abstract syntax tree. The dlrc NesC parser generates for each source file such an AST. It is a first abstraction of the source code, based on the NesC syntax.}
...
\nomenclature{Dlrc}{The TinyOS 2.x Plug-in for Eclipse, nicknamed "Dlrc 2", was developed by the Distributed Computing Group at DLRC. The plug-in aims to provide developers with all the convenience functions expected from a modern development environment.}
\renewcommand{\nomname}{Glossary}
\printnomenclature
